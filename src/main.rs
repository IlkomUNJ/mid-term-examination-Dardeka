...
fn test_binary_search_tree(){
    ...
    rootlink.borrow_mut().add_node(&rootlink, 6);
    rootlink.borrow_mut().add_node(&rootlink, 18);
    rootlink.borrow_mut().add_node(&rootlink, 17);
    rootlink.borrow_mut().add_node(&rootlink, 20);
    rootlink.borrow_mut().add_node(&rootlink, 3);
    rootlink.borrow_mut().add_node(&rootlink, 7);
    rootlink.borrow_mut().add_node(&rootlink, 2);
    rootlink.borrow_mut().add_node(&rootlink, 4);
    rootlink.borrow_mut().add_node(&rootlink, 13);
    rootlink.borrow_mut().add_node(&rootlink, 9);
    ...
    // // TESTING FOR SUCCESSOR AND PREDECESSOR
    // let target = rootlink.borrow().tree_search(&6);
    // let succ = BstNode::tree_successor(&target.clone().unwrap());
    // println!("The successor is : {:#?}", succ);
    
    // let pre = BstNode::tree_predecessor(&target.unwrap());
    // println!("The predes is : {:#?}", pre);
    ...
    // println!("The parent of min node is {:#?}", min_node.clone().borrow().parent.clone().unwrap().upgrade());
    ...
    //     // 22 // non-existent key
    // ];
    let nodes = rootlink.borrow().median();
